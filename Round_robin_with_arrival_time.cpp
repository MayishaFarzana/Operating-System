...
vector<int>seq;
vector<int>tseq;
void check_arrival_time(int t)
{
    for (int i = 0; i < number_of_process; i++)
    {
        if (ar[i].arrival_time <= t && !queued[i])
        {
            q.push(ar[i]);
            queued[i] = true;
        }
...
void run_process(int t)
{
    ...
    if (q.front().cpu_time_left <= 0)
    {
        ...
        tseq.push_back(t);
        ar[q.front().id - 1].turnarround_time = t - q.front().arrival_time;
        ar[q.front().id - 1].done = true;
        q.pop();
        finished++;
    }
    if (q.empty())
    ...
    if (q.front().curr_running_time >= 3)
    {
        seq.push_back(q.front().id);
        tseq.push_back(t);
